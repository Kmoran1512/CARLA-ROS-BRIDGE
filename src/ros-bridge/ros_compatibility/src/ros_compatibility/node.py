    ...
    class CompatibleNode(object):
        ...
        def new_publisher(self, msg_type, topic, qos_profile, callback_group=None):
            ...
            return rospy.Publisher(
                ...
                latch=qos_profile.is_latched(),
            )

        def new_subscription(
            self, msg_type, topic, callback, qos_profile, callback_group=None
        ):
            if isinstance(qos_profile, int):
                qos_profile = ros_compatibility.qos.QoSProfile(depth=qos_profile)

            return rospy.Subscriber(
                topic, msg_type, callback, queue_size=qos_profile.depth
            )
        ...
        def new_service(
            self, srv_type, srv_name, callback, qos_profile=None, callback_group=None
        ):
        ...
        def call_service(
            self, client, req, timeout=None, spin_until_response_received=False
        ):
            try:
                return client(req)
            except rospy.ServiceException as e:
    ...
    class CompatibleNode(Node):
        ...
        def get_param(self, name, alternative_value=None):
            return self.get_parameter_or(
                name, Parameter(name, value=alternative_value)
            ).value
        ...
        def new_publisher(self, msg_type, topic, qos_profile, callback_group=None):
            ...
            return self.create_publisher(
                msg_type, topic, rclpy_qos_profile, callback_group=callback_group
            )
                ...
                if future.done():
                    ...
                    if timeout is not None:
                        raise ServiceException(
                            "Service did not return a response before timeout {}".format(
                                timeout
                            )
                        )
                    else:
