...
class ScenarioRunnerRunner(ApplicationRunner):
    ...
    def __init__(
        self, path, host, port, wait_for_ego, status_updated_fct, log_fct
        ...
        super(ScenarioRunnerRunner, self).__init__(
            status_updated_fct, log_fct, "ScenarioManager: Running scenario "
        )
    ...
    def execute_scenario(self, scenario_file):
        ...
        scenario_version = (
            "--openscenario" if scenario_file.endswith(".xosc") else "--openscenario2"
        )
        if ROS_VERSION == 1:
        ...
        cmdline = [
            python_path,
            "{}/scenario_runner.py".format(self._path),
