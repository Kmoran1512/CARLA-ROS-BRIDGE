...
OBJECT_LABELS = {
    ...
    carla.CityObjectLabel.Buildings: ColorRGBA(
        r=70.0 / 255.0, g=70.0 / 255.0, b=70.0 / 255.0, a=0.8
    ),
    carla.CityObjectLabel.Fences: ColorRGBA(
        r=100.0 / 255.0, g=40.0 / 255.0, b=40.0 / 255.0, a=0.8
    ),
    ...
    carla.CityObjectLabel.Poles: ColorRGBA(
        r=153.0 / 255.0, g=153.0 / 255.0, b=153.0 / 255.0, a=0.8
    ),
    carla.CityObjectLabel.RoadLines: ColorRGBA(
        r=157.0 / 255.0, g=234.0 / 255.0, b=50.0 / 255.0, a=0.8
    ),
    carla.CityObjectLabel.Roads: ColorRGBA(
        r=128.0 / 255.0, g=64.0 / 255.0, b=128.0 / 255.0, a=0.8
    ),
    carla.CityObjectLabel.Sidewalks: ColorRGBA(
        r=244.0 / 255.0, g=35.0 / 255.0, b=232.0 / 255.0, a=0.8
    ),
    carla.CityObjectLabel.Vegetation: ColorRGBA(
        r=107.0 / 255.0, g=142.0 / 255.0, b=35.0 / 255.0, a=0.8
    ),
    # carla.CityObjectLabel.Vehicles: ColorRGBA(r=0.0/255.0, g=0.0/255.0, b=142.0/255.0, a=0.8),
    carla.CityObjectLabel.Walls: ColorRGBA(
        r=102.0 / 255.0, g=102.0 / 255.0, b=156.0 / 255.0, a=0.8
    ),
    carla.CityObjectLabel.TrafficSigns: ColorRGBA(
        r=220.0 / 255.0, g=220.0 / 255.0, b=0.0 / 255.0, a=0.8
    ),
    ...
    carla.CityObjectLabel.Bridge: ColorRGBA(
        r=150.0 / 255.0, g=100.0 / 255.0, b=100.0 / 255.0, a=0.8
    ),
    carla.CityObjectLabel.RailTrack: ColorRGBA(
        r=230.0 / 255.0, g=150.0 / 255.0, b=140.0 / 255.0, a=0.8
    ),
    carla.CityObjectLabel.GuardRail: ColorRGBA(
        r=180.0 / 255.0, g=165.0 / 255.0, b=180.0 / 255.0, a=0.8
    ),
    carla.CityObjectLabel.TrafficLight: ColorRGBA(
        r=250.0 / 255.0, g=170.0 / 255.0, b=30.0 / 255.0, a=0.8
    ),
...
class MarkerSensor(PseudoActor):
    ...
    def __init__(self, uid, name, parent, node, actor_list, world):
        ...
        super(MarkerSensor, self).__init__(uid=uid, name=name, parent=parent, node=node)
        self.actor_list = actor_list
        self.world = world
        self.node = node

        self.marker_publisher = node.new_publisher(
            MarkerArray, self.get_topic_prefix(), qos_profile=10
        )
    ...
    def _get_marker_from_environment_object(self, environment_object):
        ...
        marker.color = OBJECT_LABELS.get(
            environment_object.type, ColorRGBA(r=1.0, g=0.0, b=0.0, a=0.8)
        )
        return marker
