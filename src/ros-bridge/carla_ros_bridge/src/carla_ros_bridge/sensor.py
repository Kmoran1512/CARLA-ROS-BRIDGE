...
class Sensor(Actor):
    ...
    def get_ros_transform(self, pose, timestamp):
        if self.synchronous_mode:
            if not self.relative_spawn_pose:
                self.node.logwarn(
                    "{}: No relative spawn pose defined".format(self.get_prefix())
                )
                return
    ...
    def publish_tf(self, pose, timestamp):
        ...
        try:
            ...
            if roscomp.ok():
                self.node.logwarn(
                    "Sensor {} failed to send transform.".format(self.uid)
                )
    ...
    def _callback_sensor_data(self, carla_sensor_data):
        ...
        if self.synchronous_mode:
            ...
            try:
                ...
                if roscomp.ok():
                    self.node.logwarn(
                        "Sensor {}: Error while executing sensor_data_updated().".format(
                            self.uid
                        )
                    )
            ...
            while True:
                try:
                    ...
                    if carla_sensor_data.frame == frame:
                        ...
                        self.node.logwarn(
                            "{}({}): skipping old frame {}, expected {}".format(
