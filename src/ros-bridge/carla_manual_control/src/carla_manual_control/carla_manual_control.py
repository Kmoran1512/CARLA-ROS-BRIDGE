...
class ManualControl(CompatibleNode):
    ...
    def __init__(self, resolution):
        ...
        self.controller = JoystickControl(self.role_name, self.hud, self)
...
class JoystickControl(object):
    ...
    def _set_joystick_dimensions(self):
    ...
    def _handle_joystick_buttons(self):
        if not self._key_cache:
            if any(
                ...
                self.manual_override = not self.manual_override
                self.set_manual_override(self.manual_override)
            elif self._joystick.get_button(self._reverse_idx):
                self._key_cache = True
                self.m_ctrl.gear = 1 if self.m_ctrl.reverse else -1
...
class HUD(object):
    ...
    def __init__(self, role_name, width, height, node):
        ...
        self.status_subscriber = node.new_subscription(
            CarlaStatus, "/carla/status", self.carla_status_updated, qos_profile=10
        )
            ...
            for item in self._info_text:
                ...
                if isinstance(item, list):
                    ...
                    if isinstance(item[1], bool):
                        ...
                        if item[2] < 0.0:
                            rect = pygame.Rect(
                                (bar_h_offset + int(f * (bar_width - 6)), v_offset + 8),
                                (6, 6),
                            )
                        else:
                            rect = pygame.Rect(
                                (bar_h_offset, v_offset + 8), (int(f * bar_width), 6)
                            )
