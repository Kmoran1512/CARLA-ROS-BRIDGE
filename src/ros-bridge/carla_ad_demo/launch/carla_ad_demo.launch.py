...
def launch_carla_spawn_object(context, *args, **kwargs):
    ...
    spawn_point_param_name = "spawn_point_" + launch.substitutions.LaunchConfiguration(
        "role_name"
    ).perform(context)
...
def launch_target_speed_publisher(context, *args, **kwargs):
    ...
    data_string = (
        "{'data': "
        + launch.substitutions.LaunchConfiguration("target_speed").perform(context)
        + "}"
    )
    return [
...
def generate_launch_description():
    ld = launch.LaunchDescription(
        [
            ...
            launch.actions.DeclareLaunchArgument(
                name="spawn_point", default_value="339.0,-244.4,2.0,0,0,90"
            ),
            launch.actions.DeclareLaunchArgument(
            ...
            launch.actions.IncludeLaunchDescription(
                launch.launch_description_sources.PythonLaunchDescriptionSource(
                    os.path.join(
                        get_package_share_directory("carla_ad_agent"),
                        "carla_ad_agent.launch.py",
                    )
                ),
                launch_arguments={
